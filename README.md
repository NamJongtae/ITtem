...
- [🔨 리팩토링](#-리팩토링)
  ...
  - [🔮 무한 스크롤 @tanstack/react-virtual 가상 스크롤 적용](#-무한-스크롤-tanstackreact-virtual-가상-스크롤-적용)
  - [🔍 SEO & UX 최적화를 위한 하이브리드 렌더링 전략 (RSC + CSR)](#-seo--ux-최적화를-위한-하이브리드-렌더링-전략-rsc--csr)
...
#### 🔮 무한 스크롤 @tanstack/react-virtual 가상 스크롤 적용

> **적용이유**

- 무한 스크롤 방식을 사용하면서, 스크롤이 길어질수록 DOM 노드가 지속적으로 증가합니다.
- 이로 인해 일정 개수 이상부터는 렌더링 비용 증가 및 스크롤 성능 저하로 이어질 수 있기 때문입니다.

> **적용 방법**

- @tanstack/react-virtual을 사용해 viewport 기반 가상 리스트를 구현하였습니다.
- 전체 아이템 개수는 유지하되, 실제 렌더링되는 요소 수를 제한하여 성능을 최적화 하였습니다.


> **적용으로 얻은 이점**
- DOM 노드 수 대폭 감소: 스크롤 길이에 관계없이 일정 수의 컴포넌트만 유지
- 스크롤 성능 및 렌더링 성능 개선: 긴 목록에서도 끊김 없는 스크롤 경험 제공
- 메모리 사용량 감소: 불필요한 컴포넌트 마운트/언마운트 최소화

> **참고**

[velog-무한 스크롤 성능 최적화](https://velog.io/@njt6419/%EB%AC%B4%ED%95%9C-%EC%8A%A4%ED%81%AC%EB%A1%A4-%EC%84%B1%EB%8A%A5-%EC%B5%9C%EC%A0%81%ED%99%94-DevTools%EB%A1%9C-%ED%99%95%EC%9D%B8%ED%95%98%EA%B3%A0-TanStack-Virtualtanstackreact-virtual%EB%A1%9C-%ED%95%B4%EA%B2%B0%ED%95%98%EA%B8%B0)



<br/>

#### 🔍 SEO & UX 최적화를 위한 하이브리드 렌더링 전략 (RSC + CSR)

> **적용이유**
- SPA/CSR 중심 구조에서는 검색 엔진 크롤러가 상품명, 가격, 상태, 링크 등 핵심 콘텐츠를 즉시 인식하기 어려운 문제가 존재합니다.
- SEO가 중요한 페이지(메인 홈, 상품 목록, 상품 상세, 공개 프로필)에서는 크롤러가 반드시 읽어야 할 콘텐츠를 서버에서 완성된 HTML로 제공할 필요가 있다고 판단하였습니다.
- 이에 따라 SEO 핵심 마크업은 Server Component, 인터랙션 중심 UI는 Client Component로 분리하였습니다.

> **적용 방법**
1) 메인 페이지
    - 추천 상품 무한 스크롤의 초기 첫 페이지를 Server Component로 렌더링
    - 이후 데이터는 React Query(useInfiniteQuery) 기반 CSR 무한 스크롤 처리

2) 상품 페이지
    - ?category= query string 대신 /products/categories/[category_id] 동적 라우트 구조로 전환
    - 카테고리별 상품 목록 무한 스크롤 첫 페이지를 Server Component로 구성 이후 데이터는 React Query(useInfiniteQuery) 기반 CSR 무한 스크롤 처리

3) 상품 상세 페이지
    - 상품 이름, 가격, 설명 등 SEO 핵심 영역을 Server Component로 분리
    - 찜 수, 조회 수, 판매자 정보 등 영역은 Client Component로 처리

4) 유저 프로필 페이지(공개)
    - 유저 프로필 정보(닉네임/소개/프로필 이미지 등)를 Server Component로 구성
    - 탭 메뉴 및 탭 내부 데이터 목록은 Client Component로 분리

> **적용으로 얻은 이점**
- SEO 개선: 크롤러가 초기 HTML에서 핵심 콘텐츠를 즉시 수집 가능, 검색 노출 품질 향상
- 사용자 체감 로딩 속도 개선: 첫 페이지 핵심 콘텐츠를 서버에서 바로 렌더링하여 초기 화면이 빠르게 표시
