...
describe("addProductWish API 함수 테스트", () => {
  ...
  it("PATCH 요청을 보내고 응답을 반환합니다.", async () => {
    ...
    const result = await addProductWish(mockProductId);
  ...
  it("요청 중 에러가 발생하면 예외를 던집니다.", async () => {
    ...
    await expect(addProductWish(mockProductId)).rejects.toThrow(
      "상품 찜에 실패했어요.\n잠시 후 다시 시도해주세요."
    );
  });
});
