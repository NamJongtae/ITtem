...
import usePurchaseCancelRequestWithdrawalMutate from "@/domains/product/manage/hooks/mutations/usePurchaseCancelRequestWithdrawalMutate";
...
jest.mock("@/domains/product/manage/api/purchaseCancelRequestWithdrawal");
...
describe("usePurchaseCancelRequestWithdrawalMutate 훅 테스트", () => {
  ...
  const mockPurchaseCancelRequestWithdrawal =
    purchaseCancelRequestWithdrawal as jest.Mock;
  ...
  it("onSuccess 시 toast.success, invalidateQueries, stopLoading을 호출합니다.", async () => {
    const mockMessage = "구매 취소 요청 철회 완료";
    mockPurchaseCancelRequestWithdrawal.mockResolvedValue({
    ...
    act(() => {
      result.current.purchaseCancelRequestWithdrawalMutate("product123");
    });
  ...
  it("onError 시 axios 에러일 경우 toast.warn, stopLoading을 호출합니다.", async () => {
    ...
    mockPurchaseCancelRequestWithdrawal.mockRejectedValue(error);
