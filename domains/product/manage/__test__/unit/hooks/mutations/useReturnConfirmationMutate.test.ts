...
import useReturnConfirmationMutate from "@/domains/product/manage/hooks/mutations/useReturnConfirmationMutate";
...
jest.mock("@/domains/product/manage/api/productReturnConfirmation");
...
describe("useReturnConfirmationMutate 훅 테스트", () => {
  ...
  const mockProductReturnConfirmation = productReturnConfirmation as jest.Mock;
  ...
  it("onSuccess 시 toast.success, invalidateQueries, stopLoading을 호출합니다.", async () => {
    const mockMessage = "반품 요청이 확인되었습니다.";
    mockProductReturnConfirmation.mockResolvedValue({
      data: { message: mockMessage }
    });

    const invalidateSpy = jest.spyOn(queryClient, "invalidateQueries");

    const { result } = renderHook(() => useReturnConfirmationMutate(), {
      wrapper
    });

    act(() => {
      result.current.productReturnConfirmationMutate("product456");
    });
  ...
  it("onError 시 axios 에러일 경우 toast.warn, stopLoading을 호출합니다.", async () => {
    const error = {
      isAxiosError: true,
      response: { data: { message: "반품 오류 발생" } }
    };
    mockProductReturnConfirmation.mockRejectedValue(error);
    ...
    await waitFor(() => {
      expect(toast.warn).toHaveBeenCalledWith("반품 오류 발생");
      expect(stopLoading).toHaveBeenCalled();
    });
  });
