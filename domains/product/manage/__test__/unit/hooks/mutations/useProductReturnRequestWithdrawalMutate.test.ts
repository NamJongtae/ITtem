...
import useProductReturnRequestWithdrawalMutate from "@/domains/product/manage/hooks/mutations/useProductReturnRequestWithdrawalMutate";
import productReturnRequestWithdrawal from "@/domains/product/manage/api/productReturnRequestWithdrawal";
import { toast } from "react-toastify";
import { queryKeys } from "@/shared/common/query-keys/queryKeys";
import { createQueryClientWrapper } from "@/shared/__mocks__/utils/testQueryClientWrapper";
import useGlobalLoadingStore from "@/shared/common/store/globalLogingStore";

jest.mock("@/domains/product/manage/api/productReturnRequestWithdrawal");
...
describe("useProductReturnRequestWithdrawalMutate 훅 테스트", () => {
  ...
  const mockProductReturnRequestWithdrawal =
    productReturnRequestWithdrawal as jest.Mock;
  ...
  it("onSuccess 시 toast.success, invalidateQueries, stopLoading을 호출합니다.", async () => {
    const mockMessage = "반품 요청 철회 완료";
    mockProductReturnRequestWithdrawal.mockResolvedValue({
    ...
    act(() => {
      result.current.productReturnRequestWithdrawalMutate("product123");
    });
  ...
  it("onError 시 axios 에러일 경우 toast.warn을 호출합니다.", async () => {
    const error = {
      isAxiosError: true,
      response: { data: { message: "에러 발생" } }
    };
    mockProductReturnRequestWithdrawal.mockRejectedValue(error);
