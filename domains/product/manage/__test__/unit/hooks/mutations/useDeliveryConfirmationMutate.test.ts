...
import useDeliveryConfirmationMutate from "@/domains/product/manage/hooks/mutations/useDeliveryConfirmationMutate";
import productDeliveryConfirmation from "@/domains/product/manage/api/productDeliveryComfirmation";
import { toast } from "react-toastify";
import { queryKeys } from "@/shared/common/query-keys/queryKeys";
import { createQueryClientWrapper } from "@/shared/__mocks__/utils/testQueryClientWrapper";
import useGlobalLoadingStore from "@/shared/common/store/globalLogingStore";

jest.mock("@/domains/product/manage/api/productDeliveryComfirmation");
...
describe("useDeliveryConfirmationMutate 훅 테스트", () => {
  ...
  const mockProductDeliveryConfirmation =
    productDeliveryConfirmation as jest.Mock;
  ...
  it("onMutate에서 startLoading이 호출됩니다.", async () => {
    const { result } = renderHook(() => useDeliveryConfirmationMutate(), {
      wrapper
    });

    act(() => {
      result.current.productDeliveryConfirmationMutate("product123");
    });
  ...
  it("onSuccess 시 toast.success, invalidateQueries, stopLoading을 호출합니다.", async () => {
    const mockMessage = "배송 완료 확인";
    mockProductDeliveryConfirmation.mockResolvedValue({
  ...
  it("onError 시 axios 에러일 경우 toast.warn, stopLoading을 호출합니다.", async () => {
    ...
    mockProductDeliveryConfirmation.mockRejectedValue(error);
