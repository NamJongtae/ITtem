...
describe("productReturnRequestWithdrawal", () => {
  ...
  it("productId를 포함한 PATCH 요청을 보내고 응답을 반환합니다.", async () => {
    ...
    const result = await productReturnRequestWithdrawal(mockProductId);

    expect(customAxios.patch).toHaveBeenCalledWith(
      `/api/trading/purchase/${mockProductId}/return/withdrawal`
    );
  ...
  it("요청 중 에러가 발생하면 예외를 던집니다.", async () => {
    ...
    await expect(productReturnRequestWithdrawal(mockProductId)).rejects.toThrow(
      error
    );
  });
});
