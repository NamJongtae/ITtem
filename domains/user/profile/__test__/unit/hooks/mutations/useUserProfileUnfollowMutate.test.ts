...
describe("useUserProfileFollowMutate 훅 테스트", () => {
  ...
  it("onMutate: target 4개 cancel + optimistic 업데이트 수행", async () => {
    ...
    await waitFor(() => {
      ...
      // targetProfile followersCount +1 (isFollow는 훅에서 변경 안 함)
      ...
      // targetFollowers: 내 카드가 맨 앞에 삽입 + 내 followingsCount +1
      ...
      // targetFollowings: 내 카드 followingsCount +1
  ...
  it("onError: previousFollowStatus undefined면 follow-status removeQueries + toast.warn(409)", async () => {
  ...
  it("onError: status 409가 아니면 일반 실패 toast.warn", async () => {
  ...
  it("onSettled: follow-status만 invalidateQueries", async () => {
    mockFollowUser.mockResolvedValue({ message: "ok" });
