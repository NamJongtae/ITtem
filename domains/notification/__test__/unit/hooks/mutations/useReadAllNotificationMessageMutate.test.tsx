...
jest.mock("@/domains/notification/api/readAllNotificationMessage");

const mockReadAll = readAllNotificationMessage as jest.Mock;

describe("useReadAllNotificationMessagesMutate 훅 테스트", () => {
  ...
  it("mutate가 성공하면 캐시 데이터의 모든 메시지가 읽음 처리되고 invalidateQueries가 호출됩니다.", async () => {
    mockReadAll.mockResolvedValue(undefined);

    const { result } = renderHook(
      () => useReadAllNotificationMessagesMutate(),
      {
        wrapper
      }
    );
