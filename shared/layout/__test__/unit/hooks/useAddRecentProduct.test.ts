...
describe("useAddRecentProduct", () => {
  it("recentProductData를 localStorage에 저장합니다.", () => {
  ...
  it("새로운 상품이 추가되면 기존 상품목록 맨 앞에 추가됩니다.", () => {
    const recentProductData: RecentProductData = {
      productId: "2",
      productName: "새로운 상품",
  ...
  it("이미 있는 상품은 중복 제거하고 맨 앞에 추가해야 합니다.", () => {
    const recentProductData: RecentProductData = {
      productId: "1",
      productName: "중복 상품",
  ...
  it("최근 상품이 10개일 경우 새 항목 추가 후 가장 오래된 항목을 제거해야 합니다.", () => {
  ...
  it("productDetailData가 undefined면 아무 동작도 하지 않아야 합니다.", () => {
